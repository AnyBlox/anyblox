...
pub unsafe extern "C" fn anyblox_decode(
    ...
    let first_name_dec = if projection.contains(0) {
        decode_column(
    ...
    let last_name_dec = if projection.contains(1) {
        decode_column(
    ...
    let state_dec = if projection.contains(2) {
        decode_column(
            &mut state.state_col,
    ...
    write(tuple_count, first_name_dec, last_name_dec, state_dec, &raw mut BATCH);
