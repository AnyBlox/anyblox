...
#[derive(Default)]
pub struct NativeRleSimdSse2Impl {
    state: Option<State>,
}
...
impl NativeImpl for NativeRleSimdSse2Impl {
    fn anyblox_decode(
        &mut self,
        data: &[u8],
        start_tuple: usize,
...
impl Int32ColumnBuffer {
    ...
    pub unsafe fn write_times(&mut self, val_ptr: *const u32, times: usize) {
        ...
        let elem_v = unsafe { _mm_set1_epi32(elem as i32) };
        let mut rem_times = times as i32;

        while rem_times > 0 {
            unsafe { _mm_storeu_si128(self.ptr.add(self.len * 4).cast(), elem_v) };
            self.len += 4;
            rem_times -= 4;
        }
